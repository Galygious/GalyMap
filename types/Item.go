...
const (
	IFLAG_TARGET       uint32 = 0x00000002
	IFLAG_TARGETING    uint32 = 0x00000004
	IFLAG_TARGET_2     uint32 = 0x00000008
	IFLAG_IDENTIFIED   uint32 = 0x00000010
	IFLAG_QUANTITY     uint32 = 0x00000020
	IFLAG_SWITCHIN     uint32 = 0x00000040
	IFLAG_SWITCHOUT    uint32 = 0x00000080
	IFLAG_BROKEN       uint32 = 0x00000100
	IFLAG_REPAIRED     uint32 = 0x00000200
	IFLAG_UNK1         uint32 = 0x00000400
	IFLAG_SOCKETED     uint32 = 0x00000800
	IFLAG_NOSELL       uint32 = 0x00001000
	IFLAG_INSTORE      uint32 = 0x00002000
	IFLAG_NOEQUIP      uint32 = 0x00004000
	IFLAG_NAMED        uint32 = 0x00008000
	IFLAG_ISEAR        uint32 = 0x00010000
	IFLAG_STARTITEM    uint32 = 0x00020000
	IFLAG_INIT         uint32 = 0x00080000
	IFLAG_ETHEREAL     uint32 = 0x00400000
	IFLAG_PERSONALIZED uint32 = 0x01000000
	IFLAG_LOWQUALITY   uint32 = 0x02000000
	IFLAG_RUNEWORD     uint32 = 0x04000000
...
func NewItem(txtFileNo int, qualityNo int, uniqueOrSetId int) *Item {
	...
	item.Name = GetItemBaseName(txtFileNo)
	item.SetQuality(qualityNo)
	item.LocalizedName = GetLocalizedNameFromTxt(txtFileNo)

	if item.Quality == "Set" {
		itemCode := GetItemCodeFromTxt(txtFileNo)
		if uniqueOrSetId != 0 {
			setName := GetSetNameFromId(uniqueOrSetId)
			item.PrefixName = setName
		} else {
			setName := GetSetItemName(itemCode)
			if setName != "" {
				item.PrefixName = setName
			}
		}
		...
		itemCode := GetItemCodeFromTxt(txtFileNo)
		if uniqueOrSetId != 0 {
			uniqueName := GetUniqueNameFromId(uniqueOrSetId)
			item.PrefixName = uniqueName
		} else {
			uniqueName := GetUniqueItemName(itemCode)
			if uniqueName != "" {
				item.PrefixName = uniqueName
			}
		}
...
func GetQuality(qualityNo int) string {
	if quality, exists := qualityMap[qualityNo]; exists {
		return quality
	}
	return ""
}

func GetSetNameFromId(uniqueOrSetId int) string {
	if name, exists := setNameIdMap[uniqueOrSetId]; exists {
		return name
	}
	return ""
}

func GetUniqueNameFromId(uniqueOrSetId int) string {
	if name, exists := uniqueNameIdMap[uniqueOrSetId]; exists {
		return name
	}
	return ""
}

func GetLocalizedNameFromTxt(txtFileNo int) string {
	if name, exists := localizedNameMap[txtFileNo]; exists {
		return name
	}
	return ""
}

func GetItemCodeFromTxt(txtFileNo int) string {
	if itemcode, exists := itemCodeMap[txtFileNo]; exists {
		return itemcode
	}
	return ""
}

func GetSetItemName(itemCode string) string {
	if setItemName, exists := setItemNameMap[itemCode]; exists {
		return setItemName
	}
	return ""
}

func GetUniqueItemName(itemCode string) string {
	if uniqueItemName, exists := uniqueItemNameMap[itemCode]; exists {
		return uniqueItemName
	}
	return ""
}

func GetItemBaseName(txtFileNo int) string {
	if itemBaseName, exists := itemBaseNameMap[txtFileNo]; exists {
		return itemBaseName
	}
	return ""
}

func IsQuestItem(txtFileNo int) bool {
	switch txtFileNo {
	case 87, 88, 89, 90, 91, 92, 173, 521, 524, 549, 550, 551, 552, 553, 554, 555:
		return true
	default:
		return false
	}
...
func (item *Item) SetQuality(qualityNo int) {
	item.Quality = GetQuality(qualityNo)
}

func (item *Item) SetSetNameFromId(uniqueOrSetId int) {
	item.PrefixName = GetSetNameFromId(uniqueOrSetId)
}

func (item *Item) SetUniqueNameFromId(uniqueOrSetId int) {
	item.PrefixName = GetUniqueNameFromId(uniqueOrSetId)
}

func (item *Item) SetLocalizedNameFromTxt(txtFileNo int) {
	item.PrefixName = GetLocalizedNameFromTxt(txtFileNo)
}

func (item *Item) SetItemCodeFromTxt(txtFileNo int) {
	item.PrefixName = GetItemCodeFromTxt(txtFileNo)
}

func (item *Item) SetSetItemName(itemCode string) {
	item.Name = GetSetItemName(itemCode)
}

func (item *Item) SetUniqueItemName(itemCode string) {
	item.Name = GetUniqueItemName(itemCode)
}

func (item *Item) SetIsQuestItem(txtFileNo int) {
	item.IsQuestItem = IsQuestItem(txtFileNo)
}
...
func (item *Item) CalculateFlags(flags uint32) string {
	...
	if flags&IFLAG_TARGET != 0 { // 0x00000002
		flagsList = append(flagsList, "IFLAG_TARGET")
		// Set any item properties if necessary
	}

	if flags&IFLAG_TARGETING != 0 { // 0x00000004
		flagsList = append(flagsList, "IFLAG_TARGETING")
	}

	if flags&IFLAG_TARGET_2 != 0 { // 0x00000008
		flagsList = append(flagsList, "IFLAG_TARGET_2")
	}

	if flags&IFLAG_IDENTIFIED != 0 { // 0x00000010
		item.Identified = true
		flagsList = append(flagsList, "IFLAG_IDENTIFIED")
	}

	if flags&IFLAG_QUANTITY != 0 { // 0x00000020
		flagsList = append(flagsList, "IFLAG_QUANTITY")
	}

	if flags&IFLAG_SWITCHIN != 0 { // 0x00000040
		flagsList = append(flagsList, "IFLAG_SWITCHIN")
	}

	if flags&IFLAG_SWITCHOUT != 0 { // 0x00000080
		flagsList = append(flagsList, "IFLAG_SWITCHOUT")
	}

	if flags&IFLAG_BROKEN != 0 { // 0x00000100
		flagsList = append(flagsList, "IFLAG_BROKEN")
		item.Broken = true
	}

	if flags&IFLAG_REPAIRED != 0 { // 0x00000200
		flagsList = append(flagsList, "IFLAG_REPAIRED")
		item.Repaired = true
	}

	if flags&IFLAG_UNK1 != 0 { // 0x00000400
		flagsList = append(flagsList, "IFLAG_UNK1")
	}

	if flags&IFLAG_SOCKETED != 0 { // 0x00000800
		item.IsSocketed = true
		flagsList = append(flagsList, "IFLAG_SOCKETED")
	}

	if flags&IFLAG_NOSELL != 0 { // 0x00001000
		flagsList = append(flagsList, "IFLAG_NOSELL")
	}

	if flags&IFLAG_INSTORE != 0 { // 0x00002000
		item.InStore = true
		// flagsList = append(flagsList, "IFLAG_INSTORE") // Commented in AHK code
	}

	if flags&IFLAG_NOEQUIP != 0 { // 0x00004000
		flagsList = append(flagsList, "IFLAG_NOEQUIP")
	}

	if flags&IFLAG_NAMED != 0 { // 0x00008000
		flagsList = append(flagsList, "IFLAG_NAMED")
	}

	if flags&IFLAG_ISEAR != 0 { // 0x00010000
		flagsList = append(flagsList, "IFLAG_ISEAR")
	}

	if flags&IFLAG_STARTITEM != 0 { // 0x00020000
		flagsList = append(flagsList, "IFLAG_STARTITEM")
	}

	if flags&IFLAG_INIT != 0 { // 0x00080000
		flagsList = append(flagsList, "IFLAG_INIT")
	}

	if flags&IFLAG_ETHEREAL != 0 { // 0x00400000
		item.Ethereal = true
		flagsList = append(flagsList, "IFLAG_ETHEREAL")
	}

	if flags&IFLAG_PERSONALIZED != 0 { // 0x01000000
		flagsList = append(flagsList, "IFLAG_PERSONALIZED")
	}

	if flags&IFLAG_LOWQUALITY != 0 { // 0x02000000
		flagsList = append(flagsList, "IFLAG_LOWQUALITY")
	}

	if flags&IFLAG_RUNEWORD != 0 { // 0x04000000
		item.Runeword = true
		// flagsList = append(flagsList, "IFLAG_RUNEWORD") // Commented in AHK code
	}

	if flags&IFLAG_ITEM != 0 { // 0x08000000
		flagsList = append(flagsList, "IFLAG_ITEM")
	}

	// Construct the flagsText string
	flagsText := ""
	if len(flagsList) > 0 {
		flagsText = flagsList[0]
		for _, flagName := range flagsList[1:] {
			flagsText += "," + flagName
		}
	}

	// Optionally store the flagsText in the item
	item.FlagsText = flagsText
...
var setNameIdMap = map[int]string{
	0:   "Civerb's Ward",
	1:   "Civerb's Icon",
	2:   "Civerb's Cudgel",
	3:   "Hsarus' Iron Heel",
...
var uniqueNameIdMap = map[int]string{
	0:   "The Gnasher",
	1:   "Deathspade",
	2:   "Bladebone",
	3:   "Mindrend",
...
var localizedNameMap = map[int]string{
	0:   "hax",
	1:   "axe",
	2:   "2ax",
	3:   "mpi",
...
var itemCodeMap = map[int]string{
	0:   "hax",
	1:   "axe",
	2:   "2ax",
	3:   "mpi",
...
var setItemNameMap = map[string]string{
	"lrg": "Civerb's Ward",
	"gsc": "Civerb's Cudgel",
...
var uniqueItemNameMap = map[string]string{
	"hax": "The Gnasher",
	"axe": "Deathspade",
	"2ax": "Bladebone",
	"mpi": "Mindrend",
...
var itemBaseNameMap = map[int]string{
	0:   "Hand Axe",
	1:   "Axe",
	2:   "Double Axe",
	3:   "Military Pick",
